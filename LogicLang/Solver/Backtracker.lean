...
structure ProblemContext where
  injectiveFunctions : List String := []
...
/--
-- Does a new partial assignment violate injectivity compared
-- to a specific other predicate?
--
-- i.e., f(x)=a violates injectivity if there is any existing
-- f(y)=a where x!=y in the problem context.
-/
def doesAssignmentViolateInjectivity (ass : PartialAssignment) (functionName param value : String) (ctx : ProblemContext) : Bool :=
  ctx.injectiveFunctions.contains ass.functionName
    && ass.functionName == functionName
    && ass.functionParameter != param
    && ass.assignedValue == value

partial def testConstraint (ctx : ProblemContext) (expr : PartialAssignment) (constraint : LogicalPredicate) : Bool :=
  match constraint with

    | .connect left op right =>
       match op with -- precedence: implies then and then or
        | .implies => testConstraint ctx expr (.invertedPredicate left) || testConstraint ctx expr right
        | .and => testConstraint ctx expr left && testConstraint ctx expr right
        | .or => testConstraint ctx expr left || testConstraint ctx expr right
      ...
      match (left, op, right) with
        ...
        | (.functionCall functionName (.literal x), .equals, .literal b) | (.literal b, .equals, .functionCall functionName (.literal x)) =>
          ...
          -- by the constraint, but has a different assignment value
            ...
            if doesAssignmentViolateInjectivity expr functionName x b ctx
              then False

            else if expr.functionName == functionName && expr.functionParameter == x
    ...
    | .invertedPredicate (.invertedPredicate pred) => testConstraint ctx expr pred
    ...
    | .invertedPredicate (.connect left op right) =>
      ...
      testConstraint ctx expr (.connect (.invertedPredicate left) invertedOp (.invertedPredicate right))

def testConstraints (ctx : ProblemContext) (constraints : List LogicalPredicate) (expr : PartialAssignment) : Bool :=
  constraints.all (testConstraint ctx expr)
...
def backtrack (ctx : ProblemContext) (remainingFnDomainAndRanges : List (String × String × String)) (constraints : List LogicalPredicate) (getDomainByName : String -> List String) : StateT (List LogicalPredicate) Id Unit := do
  ...
  match remainingFnDomainAndRanges with
    ...
    | (fn, x, domain) :: xs =>
      ...
      let nextAssignments := domainValues.map (λe => {
          ...
          functionParameter := x
          assignedValue := e
        }) |> List.filter (testConstraints ctx constraints)

      for p in nextAssignments do
        let updatedConstraints := substituteNewAssignmentIntoConstraints p constraints
        backtrack ctx xs ((toPred p) :: updatedConstraints) getDomainByName
